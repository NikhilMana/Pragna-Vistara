...
import {
  ...
  ChevronRight, Eye, Lightbulb, Play, XCircle,
} from 'lucide-react'
...
import { getChapter, getTopic, getAllChapters } from '@/data/physicsClass12Catalog'
...
export default function PhysicsLearningPage() {
  const navigate = useNavigate()
  const { chapterId, topicId } = useParams()

  const chapter = useMemo(() => getChapter(chapterId), [chapterId])
  const topic = useMemo(() => getTopic(chapterId, topicId), [chapterId, topicId])
  ...
  const animation = useMemo(() => topic ? getAnimation(topic.animationType) : null, [topic])
  ...
  // Check answer quality (simple keyword match)
  const checkAnswer = useCallback((question, answerText) => {
    ...
    const matched = question.expectedConcepts.filter(c => lower.includes(c.toLowerCase()))
    const missing = question.expectedConcepts.filter(c => !lower.includes(c.toLowerCase()))
    return {
      score: matched.length / question.expectedConcepts.length,
      matched, missing,
    }
  }, [])
  ...
  return (
    <div className="container-page max-w-5xl animate-fade-in">
      {/* Breadcrumb */}
      <button onClick={() => navigate(`/physics/class-12/chapters/${chapterId}`)} className="btn-ghost mb-4 pl-0">
      ...
      <div className="mb-6">
        <div className="mb-3 flex flex-wrap gap-2 text-xs">
          <span className="badge-primary">Class XII</span>
        ...
        <div className="space-y-6">
          ...
          {questions[currentQ] && (
            <div className="rounded-2xl border border-surface-border bg-surface-card/70 p-6">
              ...
              {/* Concept check */}
              {answers[questions[currentQ].id]?.trim() && (
                  ...
                  {topicNav.prev && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        navigate(`/physics/class-12/chapters/${chapterId}/topics/${topicNav.prev.id}`)
                      }}
                      className="btn-secondary"
                    >
                  ...
                  {topicNav.next && (
                    <button
                      ...
                      onClick={() => {
                        setLearningStage(0); setCurrentQ(0); setAnswers({}); setShowHints({})
                        setMisconceptionAnswers({}); setMisconceptionSubmitted(false)
                        navigate(`/physics/class-12/chapters/${chapterId}/topics/${topicNav.next.id}`)
                      }}
                      className="btn-primary"
                    >
